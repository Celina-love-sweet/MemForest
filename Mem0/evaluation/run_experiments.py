...
def main():
    ...
    if args.technique_type == "mem0":
        ...
        if args.method == "add":
            ...
            # Progress: samples and per-sample sessions
            for idx, item in tqdm(
                enumerate(memory_manager.data),
                total=len(memory_manager.data),
                desc="Samples",
                unit="sample",
            ):
                memory_manager.process_conversation(
                    item, idx, show_progress=True, total_samples=len(memory_manager.data)
                )
            ...
            # If you use a subset file, pass start_idx to preserve original indices.
            dataset_path = "dataset/locomo10.json"
            start_idx = 0
            # Example: when using only last 3 samples (orig idx 7,8,9), set start_idx=7
            memory_searcher.process_data_file(dataset_path, start_idx=start_idx)
