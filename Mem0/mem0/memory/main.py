...
class Memory(MemoryBase):
            ...
            for resp in new_memories_with_actions.get("memory", []):
                ...
                try:
                    ...
                    if event_type == "ADD":
                        ...
                        mem_key = resp.get("id")
                        if mem_key not in temp_uuid_mapping:
                            logger.warning(
                                f"Invalid memory ID '{mem_key}' returned by LLM for UPDATE. "
                                f"Available IDs: {list(temp_uuid_mapping.keys())}. Treating as ADD."
                            )
                            memory_id = self._create_memory(
                                data=action_text,
                                existing_embeddings=new_message_embeddings,
                            ...
                            actual_id = temp_uuid_mapping[mem_key]
                            self._update_memory(
                                memory_id=actual_id,
                                data=action_text,
                                existing_embeddings=new_message_embeddings,
                        ...
                        if mem_key not in temp_uuid_mapping:
                            logger.warning(
                                f"Invalid memory ID '{mem_key}' returned by LLM for DELETE. "
                                f"Available IDs: {list(temp_uuid_mapping.keys())}. Skipping DELETE."
                            )
                            continue
                        actual_id = temp_uuid_mapping[mem_key]
                        self._delete_memory(memory_id=actual_id)
                        returned_memories.append(
                            {
                                "id": actual_id,
                                "memory": action_text,
    ...
    def search(
        ...
        *,
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            if isinstance(memories_result, tuple) and len(memories_result) == 3:
                original_memories, query_time, retrieval_time = memories_result
            else:
                original_memories, query_time, retrieval_time = memories_result, None, None
        ...
        return {"results": original_memories, "query_time": query_time, "retrieval_time": retrieval_time}
    ...
    def _search_vector_store(self, query, filters, limit, threshold: Optional[float] = None):
        ...
        return original_memories, query_time, retrieval_time
            ...
            for resp in new_memories_with_actions.get("memory", []):
                ...
                try:
                    ...
                    if event_type == "ADD":
                        ...
                        if mem_key not in temp_uuid_mapping:
                            logger.warning(
                                f"Invalid memory ID '{mem_key}' returned by LLM for UPDATE (async). "
                                f"Available IDs: {list(temp_uuid_mapping.keys())}. Treating as ADD."
                            )
                            task = asyncio.create_task(
                                self._create_memory(
                        ...
                        if mem_key not in temp_uuid_mapping:
                            logger.warning(
                                f"Invalid memory ID '{mem_key}' returned by LLM for DELETE (async). "
                                f"Available IDs: {list(temp_uuid_mapping.keys())}. Skipping DELETE."
                            )
                            continue
                        actual_id = temp_uuid_mapping[mem_key]
                        task = asyncio.create_task(self._delete_memory(memory_id=actual_id))
