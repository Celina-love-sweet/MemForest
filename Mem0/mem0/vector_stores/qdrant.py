...
class Qdrant(VectorStoreBase):
    ...
    def insert(self, vectors: list, payloads: list = None, ids: list = None):
        ...
        if ids is not None and len(ids) != len(vectors):
            raise ValueError(
                f"Length mismatch: {len(ids)} ids provided for {len(vectors)} vectors; ensure ids length matches vectors."
            )
        if payloads is not None and len(payloads) != len(vectors):
            raise ValueError(
                f"Length mismatch: {len(payloads)} payloads provided for {len(vectors)} vectors; ensure payloads length matches vectors."
            )
        points = [
            PointStruct(
